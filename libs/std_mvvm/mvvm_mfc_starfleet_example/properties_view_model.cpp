...
void properties_view_model::bind_properties()
{
    ...
    main_frame_vm.setter([this](const main_frame_view_model::ptr& v) { if(_main_frame_vm.lock() != v) { unsubscribe_events(); _main_frame_vm = v; subscribe_events(); mst::wobservable_object::notify_property_changed(this->shared_from_this(), main_frame_vm.name()); } });
