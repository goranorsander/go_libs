//
//  placebo_mutex_relay_command_test_suite.cpp
//
...
TEST(std_relay_command_placebo_mutex_test_suite, cpp11_not_supported) {}
#else
...
public:
    virtual ~test_relay_command() GO_DEFAULT_DESTRUCTOR
        ...
        : mst::relay_command(TEST_RELAY_COMMAND_NAME, execute_command, can_execute_command, nullptr)
    {
    }
...
{
public:
    typedef GO_TYPENAME go::signals::signal<std::function<void(const std::shared_ptr<mst::command_interface>&)>> can_execute_changed_signal;
...
TEST(std_relay_command_placebo_mutex_test_suite, test_relay_command)
{
