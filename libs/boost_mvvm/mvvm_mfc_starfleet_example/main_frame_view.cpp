...
public:
    ...
    bool operator()(const dialog_view::ptr& v) const
    {
        return v.get() == _dialog;
    }

private:
    const dialog_view::pointer _dialog;
};
...
void main_frame_view::on_show_spaceship(const fleet_organization_id_type id)
{
    ...
    else
    {
        ...
        if(fleet_repo)
        {
            ...
            {
                spaceship_view_model::ptr spaceship_vm = spaceship_view_model::create(boost::dynamic_pointer_cast<spaceship_model>(fleet_org->spaceship_model()), id, data_context());
                LockWindowUpdate();
                const u::scope_guard unlock(boost::bind(&main_frame_view::UnlockWindowUpdate, this));
