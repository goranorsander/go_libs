...
{
protected:
    ~primitive_type_implicit_specializer_base() GO_DEFAULT_DESTRUCTOR
    primitive_type_implicit_specializer_base() GO_DEFAULT_CONSTRUCTOR
};
...
public:
    ~primitive_type_implicit_specializer() GO_DEFAULT_DESTRUCTOR
...
public:
    ...
    GO_CONSTEXPR operator const primitive_type&() const
    {
        return this->_t;
    }
    ...
    template <typename P>
    GO_CONSTEXPR typename std::enable_if<std::is_arithmetic<P>::value, bool>::type operator==(const P& p) const
    {
...
#define GO_IMPLEMENT_PRIMITIVE_TYPE_IMPLICIT_SPECIALIZER_CONSTRUCTORS( _class_name_, _primitive_type_ ) \
    _class_name_() : go::utility::primitive_type_implicit_specializer<_primitive_type_>(static_cast<_primitive_type_>(0)) {} \
