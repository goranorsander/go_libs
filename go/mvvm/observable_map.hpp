...
class basic_observable_map
    : public basic_observable_ordered_associative_container<S, std::map<K, T>, L>
{
public:
    using string_type = S;
    using lockable_type = L;
    using container_type = typename std::map<K, T>;
    using this_type = basic_observable_map<K, T, S, L>;
...
public:
    ...
    using lockable_type = L;
    using container_type = typename std::map<K, T>;
    using this_type = observable_map<K, T, L>;
    using ptr = typename std::shared_ptr<this_type>;
    using wptr = typename std::weak_ptr<this_type>;
...
public:
    ...
    using lockable_type = L;
    using container_type = typename std::map<K, T>;
    using this_type = wobservable_map<K, T, L>;
    using ptr = typename std::shared_ptr<this_type>;
    using wptr = typename std::weak_ptr<this_type>;
