...
inline typename std::enable_if<std::is_same<to_string, from_string>::value, from_string>::type string_cast(const from_string& from)
{
    return from;
}

template <typename to_string, typename from_string>
inline typename std::enable_if<!std::is_same<to_string, from_string>::value, to_string>::type string_cast(const from_string& from)
{
