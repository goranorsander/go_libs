...
void print_hex(const S& s)
{
    ...
    {
        std::wcout << s::format(hex_format, static_cast<C>(c)).c_str();
...
int main()
{
    ...
    const std::u16string utf_16 = s::string_cast<std::u16string>(utf_8_source);
    const std::u32string utf_32 = s::string_cast<std::u32string>(utf_8_source);
