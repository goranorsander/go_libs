//
//  define_go_compiler_mingw_gcc_test.ipp
//
...
TEST(std_define_go_compiler_test_suite, test_define_go_comp_gcc)
{
...
TEST(std_define_go_compiler_test_suite, test_define_go_comp_gcc_mingw)
{
...
TEST(std_define_go_compiler_test_suite, test_define_go_gcc_version)
{
    #if defined(GO_GCC_VERSION)
    const int expected_version = __GNUC__ * 10000 + __GNUC_MINOR__ * 100 + __GNUC_PATCHLEVEL__;
    const int go_gcc_version = GO_GCC_VERSION;
    EXPECT_EQ(expected_version, go_gcc_version);
    #else
    EXPECT_TRUE(false);
    #endif
}
