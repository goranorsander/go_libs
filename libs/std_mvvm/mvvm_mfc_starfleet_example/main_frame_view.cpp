...
main_frame_view::main_frame_view(const mst::wcommand_manager::ptr& command_manager, const mst::wevent_manager::ptr& event_manager, const fleet_repository::ptr& fleet_repo)
    : CMDIFrameWndEx()
...
void main_frame_view::on_close_dialog(const dialog_view::pointer dialog)
{
    if (data_context() && data_context()->dialogs() && data_context()->command_manager())
    {
        ...
        {
            mst::wcommand_interface::ptr command = data_context()->delete_dialog_view_command;
    ...
    else
    {
        ...
        if(fleet_repo)
        {
            ...
            if(fleet_org && fleet_org->spaceship_model())
            {
                ...
                child_frame_view* newChild = dynamic_cast<child_frame_view*>(CreateNewChild(RUNTIME_CLASS(child_frame_view), IDR_MVVM_MFC_STARFLEET_EXAMPLETYPE, theApp.mdiMenu(), theApp.mdiAccel()));
...
BOOL main_frame_view::CreateDockingWindows()
{
    ...
    mst::wcommand_manager::ptr command_manager = _command_manager.lock();
    if(command_manager)
    {
    ...
    mst::wevent_manager::ptr event_manager = _event_manager.lock();
    if(event_manager)
    {
