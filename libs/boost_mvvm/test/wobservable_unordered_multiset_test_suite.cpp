...
TEST(boost_wobservable_unordered_multiset_test_suite, test_erase_value)
{
    ...
    typedef m::wobservable_unordered_multiset<int> wobservable_unordered_multiset_type;
    int count = 0;
    int sum = 0;
    BOOST_FOREACH(const GO_BOOST_TYPENAME wobservable_unordered_multiset_type::value_type& i, *s)
    {
        sum += i;
        ++count;
    }
