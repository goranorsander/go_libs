...
void product_model::bind_properties()
{
    ...
    unit_price.getter([this]() -> double { return _unit_price; });
    unit_price.setter([this](const double& v) { if(v != _unit_price) { _unit_price = v; notify_property_changed(this->shared_from_this(), unit_price.name()); } });
}
